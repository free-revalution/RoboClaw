...
RoboClaw 是一个 AI Agent 框架，通过七大核心工具实现与开发环境和物理世界的交互：
...
# 列出所有对话
roboclaw conversation --list

# 切换到指定对话
roboclaw conversation --switch <conversation-id>

# 显示当前配置
roboclaw config --show
...
# 安装新技能
roboclaw skill --install <skill-file>
...
创建硬件配置文件 `~/.roboclaw/hardware.json`：
...
```bash
# 安装技能
roboclaw skill --install my_skill.json
...
使用命令：`roboclaw conversation --switch <对话ID>`

### Q2: 如何查看对话历史？

对话历史保存在 `~/.roboclaw/conversations/` 目录，以 JSON 格式存储。
